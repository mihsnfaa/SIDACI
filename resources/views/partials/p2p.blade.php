...
<div class="p-6 bg-gray-100 rounded-lg shadow">
    <h2 class="mb-4 text-2xl font-semibold">p2p</h2>

    {{-- Upload Form: hanya tampil untuk admin dan user bidang p2p --}}
    @if (auth()->user()->level === 'admin' || auth()->user()->bidang === 'p2p')
        <form action="{{ route('dokumen.upload') }}" method="POST" enctype="multipart/form-data" class="p-4 mb-6 space-y-4 bg-white rounded shadow">
            @csrf
            <input type="hidden" name="bidang" value="p2p">
